    ...
    variables.forEachIndexed { i, varName ->
      ...
      if(prop.first == null) {
        ...
        when(prop.first) {
          ...
          is Short     -> stmt.setShort(i+1, prop.first as Short)
          is Date      -> stmt.setDate(i+1, Date.valueOf((prop.first as Date).toLocalDate()))
          is LocalDate -> stmt.setDate(i+1, Date.valueOf(prop.first as LocalDate))
          //is Array<*> -> throw Exception("Array data type not yet implemented")
          //is Collection<*> -> throw Exception("Collection data type not yet implemented")

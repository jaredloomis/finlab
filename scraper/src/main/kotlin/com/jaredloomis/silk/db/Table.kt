...
import com.jaredloomis.silk.model.realestate.RealEstateListingTable
...
abstract class Table<T>(val dbUrl: String, val tableName: String) {
  ...
  protected fun connect(): Connection {
    return if(connection != null) {
      ...
      val con = DriverManager.getConnection(dbUrl, "postgres", "")
      connection = con
      con
    }
  }
...
class ProductTable(tableName: String) : Table<Product>("jdbc:postgresql://localhost/market_analysis", tableName) {
...
class PostingTable(tableName: String, val productTable: Table<Product>)
  : Table<ProductPosting>("jdbc:postgresql://localhost/market_analysis", tableName) {
  private val logger = Logger.getLogger(this::class.simpleName)
...
val tableModule = DI.Module("Table") {
  ...
  bind<Table<RealEstateListing>>() with singleton {
    val table = RealEstateListingTable("jdbc:postgresql://localhost/market_analysis", "real_estate_listings")
    table.open()
    table
  }
}
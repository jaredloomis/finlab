...
abstract class RealEstateScraper(
  ...
  fun search(text: String): Iterator<RealEstateListing> {
    ...
    // Search
    Thread.sleep(1000)
    findElement(searchInputBy)?.sendKeys(text)
    Thread.sleep(1000)
    findElement(searchInputBy)?.sendKeys(Keys.ENTER)
    waitForPageLoad()

    // Select for sale listings
    findElement(By.cssSelector(".lightbox-body ul > li:nth-of-type(1) > button"))?.click()
    waitForPageLoad()
...
class ZillowScraper : RealEstateScraper("https://zillow.com/", Regex(".*zillow\\.com/homedetails.*"), By.cssSelector(".srp-search-box input,#search-box-input"), By.cssSelector("a[rel=next]")) {
  override fun parseListing(): RealEstateListing? {
    return try {
      ...
      val beds = bedBathSqft.find { it.contains("bd") }!!.split(" ")[0].toShort()
      val baths = bedBathSqft.find { it.contains("ba") }!!.split(" ")[0].toShort()
      ...
      RealEstateListing(
        ...
        PropertyType.HOUSE, // TODO
        beds, baths, sqft, images.toSet(), LocalDate.EPOCH, Instant.now(), emptyMap()
      )
    } catch(ex: Exception) {
      ex.printStackTrace()
      null
